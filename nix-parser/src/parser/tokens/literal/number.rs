...
pub fn float(input: Span) -> IResult<f64> {
    ...
    map_res(float, |s: Span| f64::from_str(s.fragment))(input)
}

pub fn integer(input: Span) -> IResult<i64> {
    let int = recognize(pair(opt(char('-')), digit1));
    map_res(int, |s: Span| i64::from_str(s.fragment))(input)
}
